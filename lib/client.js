window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			nav: "Anchored 预设",
			hint: "选择要安装到 ~/.dsh/.agent-presets 的 Anchored Standard preset。勾选后点击“{apply}”即可安装；取消勾选已安装的 preset 会将其卸载。",
			selectAll: "全选",
			unselectAll: "取消全选",
			apply: "应用更改",
			busy: "处理中…",
			...
			applied: "已应用 preset 安装状态。",
		};
		const en = {
			nav: "Anchored Presets",
			hint: "Choose which Anchored Standard presets to install into ~/.dsh/.agent-presets. Check items and click \"{apply}\" to install; unchecking an installed preset uninstalls it.",
			...
			applied: "Preset installation state applied.",
		};
		function fmt(text, params) {
			if (params === undefined || params === null) return text;
			return text.replace(/\{(\w+)\}/g, (m, key) => (Object.prototype.hasOwnProperty.call(params, key) ? String(params[key]) : m));
		}
		function useT(locale) {
			const subscribe = React.useCallback((cb) => (locale !== undefined && locale !== null && typeof locale.subscribe === "function" ? locale.subscribe(cb) : () => {}), [locale]);
			const getSnapshot = React.useCallback(() => (locale !== undefined && locale !== null && typeof locale.getSnapshot === "function" ? locale.getSnapshot() : null), [locale]);
			...
			return React.useCallback((key, params) => fmt(dict[key] !== undefined ? dict[key] : (en[key] !== undefined ? en[key] : key), params), [dict]);
		}
		const CSS = [
			...
			function apply() {
				...
				rpcCall(connection, "apply", { ids }).then((value) => {
					...
					if (failed.length > 0) {
						setMessage({
							kind: "err",
							text: t("partialFailed") + " " + failed.map((r) => (r !== null && typeof r === "object" ? r.id + ": " + (r.error || "unknown") : "unknown")).join("; "),
						});
					} else {
					...
					setBusy(false);
					setMessage({ kind: "err", text: t("applyFailed") + errText(err) });
				});
			}

			if (error !== null) {
				return React.createElement("div", { className: "aps-wrap" },
					React.createElement("p", { className: "aps-msg err" }, t("loadFailed") + error));
			}
			if (rows === null) {
				return React.createElement("div", { className: "aps-wrap" },
					React.createElement("span", { className: "aps-load" }, t("loading")));
			}

			const allSelected = rows.length > 0 && rows.every((row) => selected[row.id] === true);

			const children = [
				React.createElement("p", { className: "aps-hint", key: "hint" }, t("hint", { apply: t("apply") })),
			...
			for (const row of rows) {
				...
				children.push(React.createElement("div", { className: "aps-row", key: row.id },
					...
					React.createElement("span", { className: "aps-badge " + (row.installed === true ? "aps-on" : "aps-off") },
						row.installed === true ? t("installed") : t("notInstalled"))));
			}
			if (message !== null) {
		...
		function apply(ctx) {
			...
			ctx.effect(() => ctx.locale.register(NS, { zh, en }), "anchored-standard: dictionaries");
			...
			ctx.slots.inject("settings.section", () => ctx.slots.register(
				...
				(props) => React.createElement(Section, { connection: connection, locale: props !== undefined && props !== null && props.locale !== undefined ? props.locale : locale })
			));
		}
